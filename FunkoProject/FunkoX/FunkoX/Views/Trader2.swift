...
struct Trader2: View {
    ...
    var body: some View {
        HStack{
            Text("Trader - EnthusiasticPanda10")
        ...
        ScrollView(.horizontal) { //ScrollView(3)
            LazyHStack(alignment: .top, spacing: 10) {
                HStack(spacing: 15) {
                    ForEach((11...14), id: \.self) { count in
                        VStack{
                            NavigationLink(destination: MessageView(name: "EnthusiasticPanda10"), isActive: $nav6, label: { EmptyView() })
                            Button {
                                nav6 = true
                            } label: {
                                Image("FunkoX-\(count)")
...
struct Trader2_Previews: PreviewProvider {
