...
struct Trader3: View {
    ...
    @State private var nav7 = false
    var body: some View {
        HStack{
            Text("Trader - HappyCollector43")
        ...
        ScrollView(.horizontal) { //ScrollView(3)
            LazyHStack(alignment: .top, spacing: 10) {
                HStack(spacing: 15) {
                    ForEach((29...31).reversed(), id: \.self) { count in
                        VStack{
                            NavigationLink(destination: MessageView(name: "HappyCollector43"), isActive: $nav7, label: { EmptyView() })
                            Button {
                                nav7 = true
                            } label: {
                                Image("FunkoX-\(count)")
...
struct Trader3_Previews: PreviewProvider {
